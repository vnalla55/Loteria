...
$flaggreentoinsert=0;
?>
   <li class="<?php if($activelotogroup==$row->lottogroup_id) echo 'active';?>"><a href="<?php echo base_url()?>frontresult/resultbylotogroup/<?php echo $row->lottogroup_id;?>"><?php echo $row->lotto_group_name;?></a>                                      
<?php 
}
?>
   ...
   echo '<p ><h3 id="resultyetioutofyeti" style="margin-left:40%;">', $start, '-', $end, ' de ', $total, ' resultados</h3></p>';
        ?>
...
//alert(moment().subtract(29, 'days').format('MMMM D, YYYY'));
//window.location.assign('?start='+start.format('YYYY-MM-DD')+'&end='+end.format('YYYY-MM-DD'));
 ...
 if($todaykoday==1 && $row->sundayflag ==1)
     {
       $flaggreentoinsert=1;  
         ...
         $flaggreentoinsert=1; 
                             ...
                             <td>
                                  <td>
                                     <a class="popup-text btn btn-warning btn-xs lotoresultpopup" alt='<?php echo $row->lottogroup_id.'-'.$row->game_id;?>' href="#numpicker" data-effect="mfp-zoom-out" >Estadísticas</a> </td>
...
?>
                    ...
                    <div class="row">
                        ...
                        <a class="btn btn-info" href="?<?php if($startdate !=0 ){echo 'start='.$startdate.'&end='.$enddate;} ?>limit=<?php $limit+=5;  echo $limit; ?>" style="width:100%;<?php if($total==$end) echo 'display:none;'; ?>">Ver Más</a>
                         </div>
                    </div>
                    <div class="gap"></div>
                </div>
 ...
 <div id="numpicker" class="mfp-with-anim mfp-hide mfp-dialog clearfix container bg-primary" style="height:600px; max-width:1000px;" >
     <span id="graptimedisplay">3 Meses</span>
       <select id="graphtimeselector" class="pull-right" style="margin-right: 15px;">
              <option value="3">Últimos 3 Meses
